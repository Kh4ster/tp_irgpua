...
void _registerer_scan(Fixture* fx,
                      benchmark_t* b[],
                      int& index,
                      const auto& sizes,
                      bool bench_nsight,
...
{
    // Register benchmark
    b[index] = benchmark::RegisterBenchmark(
        name,
        std::bind(&Fixture::register_scan<FUNC>, fx, _1, bench_func));
    if (bench_nsight)
...
void registerer_scan(Fixture* fx,
                     benchmark_t* b[],
                     int& index,
                     const auto& sizes,
                     bool bench_nsight,
                     Tuple tuple)
{
    ...
    std::apply(
        [fx, &b, &index, &sizes, bench_nsight](auto&&... args)
        {
            _registerer_scan(fx,
                             b,
                             index,
